...
### 5b · git wrapper + 0 条默认规则 + IdleTimer(0.5d)
- [ ] `npm i simple-git`
...
- [ ] **`DEFAULT_TIER1_RULES = []`** — silent agent 默认不主动 commit。原 4 条 Tier 1 规则全部废弃(`.silent/` 整个 gitignore 后无东西可 commit):
  - ~~`chat.turn-end`~~ ❌ 删 — main_chat.jsonl 在 `.silent/runtime/` 不进 git
  - ~~`browser.load-finish`~~ ❌ 删 — latest.md 在 `.silent/` 不进 git
  - ~~`shell.exit`~~ ❌ 删 — latest-cmd.log 同理
  - `workspace.idle` 🟡 改成 opt-in 常量 `TIER1_RULES_IDLE_ONLY`,默认不启用,需用户/上层显式传 `{ rules: TIER1_RULES_IDLE_ONLY }`
- [ ] `IdleTimer` 每次 `vcs.emit` 调用时 reset(纯内存 timer,无 chokidar);默认配置下 idle event 仅写 events.jsonl 不 commit
- [ ] **新建 workspace 自动 `git init`**(为支持 worktree)+ 写默认 `.gitignore` 一行:
  ```
  .silent/
  ```
  initial commit 仅 add `.gitignore`,**不再 add `.silent/`**(整个目录是 silent agent 私域)
- [ ] pre-commit hook:文件 > 10MB 拒绝(可推 v0.2)
- [ ] tab focus 不触发 commit(只 emit events.jsonl,不写 tabs.json)
- ~~lastActiveAt 拆出 meta.yaml~~ — 已不需要:`meta.yaml` 整个不进 git(在 `.silent/` 私域),lastActiveAt 留在 meta.yaml 也无 git noise

> 设计依据更新:`.silent/` 整个不进 git 的论据 + Tier 1 砍到 0 条,详见 [`design/08-vcs.md`](design/08-vcs.md) §1 / §3 与 [`design/10-multi-agent-isolation.md`](design/10-multi-agent-isolation.md) §3.1。
...
> **`git 边界 = .silent/ 目录边界`**:整个 `.silent/` 都是 silent agent 私域(.gitignore),git 只追用户文件;`.silent/` 内部仍按 5c+ 二分(顶层放 meta.yaml + tabs/<tid>/latest.* 当"当前态视图",runtime/ 装 logs/cache/历史)。详见 [`design/08-vcs.md`](design/08-vcs.md) §1。
...
- [ ] **保留 `.silent/` 顶层文件物理位置**(`meta.yaml` / `tabs/<tid>/latest.md` / `tabs/<tid>/latest-cmd.log`),但**全部不进 git**(整个 `.silent/` gitignore)
- [ ] 一次性迁移脚本:`addWorkspace` 启动时检测旧布局(`.silent/events.jsonl` 在顶层)→ 迁到新位置 + 写迁移完成标记
- [ ] `.gitignore` 更新成单行 `.silent/`(整个 silent agent 私域不进 git)
...
- [ ] **cp 到 `.silent/tabs/<tid>/latest.md`**(❌ gitignore,但保留 copy 一份方便 main_chat 读"当前页面"视图;看页面演化走 `runtime/tabs/<tid>/snapshots/NNN-*.md` 文件序列)
- [ ] snapshot 文件头加 URL + title + ts meta(LLM 单文件可读)
- [ ] 落盘后调 `vcs.emit({source:'browser', action:'load-finish', meta:{summary, detailPath:".silent/runtime/tabs/<tid>/snapshots/NNN-..."}})` → 仅 append events.jsonl(默认 0 条 Tier 1 规则,不触发 commit)
...
- [ ] `precmd / exit` → 切片 `runtime/tabs/<tid>/snapshots/NNN-<cmd-ts>.log` + **cp 到 `.silent/tabs/<tid>/latest-cmd.log`**(❌ gitignore,保留 copy 给 main_chat 当"最近命令"视图)+ `vcs.emit({source:'shell', action:'exit', meta:{summary, cmd, exitCode, durMs, detailPath:"runtime/tabs/<tid>/snapshots/NNN-..."}})` → 仅 append events.jsonl(不触发 commit)
- [ ] 链式命令 `&&` / `;` 各自独立切片(每个 cmd 一个 NNN 文件,不再触发独立 commit)
